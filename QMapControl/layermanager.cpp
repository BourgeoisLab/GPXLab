...
namespace qmapcontrol
{
    ...
    {
        if (mapcontrol->isUpdateSuspended())
            return;
    ...
    bool LayerManager::mouseEvent(const QMouseEvent* evnt)
    {
        ...
        for (int i = mylayers.count() - 1; i >= 0; --i)
        {
            ...
            if (l && l->isVisible() )
            {
                if (l->mouseEvent(evnt, mapmiddle_px))
                    return true;
            }
        }
        return false;
    ...
    void LayerManager::resize(QSize newSize)
    {
        ...
        mapcontrol->suspendUpdate(false);
